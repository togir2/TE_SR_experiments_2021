...
class ipw(Topo):
    def build(self):
        ...
        self.add_link_name("0", "4", cost=2000, delay=0.2, bw=10000, directed=True)
        self.add_link_name("4", "0", cost=1000, delay=0.2, bw=10000, directed=True)
        self.add_link_name("1", "4", cost=1000, delay=0.2, bw=10000, directed=True)
        self.add_link_name("4", "1", cost=1000, delay=0.2, bw=10000, directed=True)
        self.add_link_name("2", "4", cost=1000, delay=0.2, bw=10000, directed=True)
        self.add_link_name("4", "2", cost=1000, delay=0.2, bw=10000, directed=True)
        self.add_link_name("3", "4", cost=1000, delay=0.2, bw=10000, directed=True)
        self.add_link_name("4", "3", cost=1000, delay=0.2, bw=10000, directed=True)
    ...
    def dijkstra_computed(self):
        ...
        for nh in nhlist:
            build_str += f" nexthop via {nh.nh} "+" encap seg6 mode inline segs {4} "+ f" weight {int(300/len(nhlist))} "
        self.add_command("0", f"ip -6 route add {{3}} metric 1 table 1 src {{0}}  {build_str}")
        self.add_command("0", "ip -6 rule add to {3/} iif lo table 1")
        self.add_command("0", "ip -6 rule add to {3/} iif lo table 1")
        self.add_command("3", "nuttcp -6 -S")
        self.add_command("0", 'echo bash -c \\\"START=\\\\\$SECONDS\; while \! ip netns exec 0 nuttcp -T150 -i1 -R40000 -N16 {3} \>\>flow_0-3.txt 2\>\&1 \; do sleep 1\; echo RTY\: \\\\\$SECONDS \>\>flow_0-3.txt\; done\\\" | at now+2min')
        self.add_command("0", 'echo bash -c \\\"START=\\\\\$SECONDS\; while \! ip netns exec 0 nuttcp -T150 -i1 -R40000 -N16 {3} \>\>flow_0-3.txt 2\>\&1 \; do sleep 1\; echo RTY\: \\\\\$SECONDS \>\>flow_0-3.txt\; done\\\" | at now+2min')
